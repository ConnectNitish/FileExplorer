...
void copy_file( char *filename, char *dirname , char * to_append,char *currentWorkingDir);
	...
	strcat(Path,"/");                 //Copies the current path to the 'Path' variable.
		...
		printf("stat val =%d\n",stat(Path, &info));
				...
				MultipleSearchDirectory(Path,tardir,currentWorkingDir);   //Calls this function AGAIN, this time with the sub-name.
                    						...
                    						//Run Copy routine
					printf("fname is = %s ",e->d_name);
					copy_file1( e->d_name, tardir , Path,currentWorkingDir);
               		}
            	}
        }
    }
		...
		printf("not a dir fname is = %s\n ", spath);
		copy_file1( spath, tardir,NULL,currentWorkingDir);
	}
}
void copy_file1( char *filename, char *dirname , char * to_append,char *currentWorkingDir){
